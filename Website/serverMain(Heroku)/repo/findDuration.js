...
const findDuration = (path, graph, durationPerTransfer) => {
  ...
  for (let i = 0; i < path.length; i++) {
    if (i == 0 || i == 1) {
      ...
      if (i === 1) {
        const BusService = path[i].substring(index2 + 1, path[i].length);
        services.push(BusService);
      }
    } else {
      ...
      const currBusService = path[i - 1].substring(
        index + 1,
        path[i - 1].length
      );
  ...
  return [
    duration + numberOfBusTransfers * durationPerTransfer,
    numberOfBusTransfers,
    services,
  ];
};
